...
function openVideoPlayerApp(fileName = null, customUrl = null) {
    const playerHTML = `
    ...
    if (fileName) {
        const override = localStorage.getItem(`wacky_file_override_${fileName}`);
        const videoUrl = customUrl || override || `${BASE_URL}${encodeURIComponent(fileName)}`;
        video.src = videoUrl;
        video.play();
    }
}

function openMediaPlayerWindow(fileName, customUrl = null) {
    ...
    if (fileName) {
        const override = localStorage.getItem(`wacky_file_override_${fileName}`);
        const mediaFileUrl = customUrl || override || `${BASE_URL}${encodeURIComponent(fileName)}`;
        if (customUrl || override) {
            content = `
                <div style="padding: 20px; display: flex; flex-direction: column; align-items: center; justify-content: center; height: 100%; background: var(--theme-bg-primary); color: var(--theme-text-primary);">
                    <div style="font-size: 48px; margin-bottom: 10px;">🎵</div>
                    <div style="font-weight: bold; margin-bottom: 15px; text-align: center; word-break: break-all;">${fileName}</div>
                    <audio src="${mediaFileUrl}" controls autoplay style="width: 100%;"></audio>
                </div>
            `;
            width = '350px';
            height = '200px';
            ...
            width = '350px';
            height = '200px';
            content = `<iframe src="${playerLink}" style="width:100%; height:100%; border:none;" title="Meaty Player"></iframe>`;
        }
    } else {
        content = `<iframe src="${playerLink}" style="width:100%; height:100%; border:none;" title="Meaty Player"></iframe>`;
    }
...
function openImageViewWindow(fileName, customUrl = null) {
    const override = localStorage.getItem(`wacky_file_override_${fileName}`);
    const imageUrl = customUrl || override || `${BASE_URL}${encodeURIComponent(fileName)}`;
